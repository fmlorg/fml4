...
sub __MemberP
{
    my ($mib, $mlfile, $addr) = @_;
    my ($query, $res, $mlname, @row);
...
sub __DumpList
{
    my ($mib, $mlfile, $ignore_off) = @_;
    my ($max, $orgf, $newf, $mlname);
    my ($query);
...
sub __ListCtl
{
	...
	$query .= " values ('$mlname', 'actives', '$addr', 0, '$NULL') ";
	&__Execute($mib, $query) || return $NULL;

	$query  = " insert into ml ";
	$query .= " values ('$mlname', 'members', '$addr', 0, '$NULL') ";
	&__Execute($mib, $query) || return $NULL;
	...
	$query  = " insert into ml ";
	$query .= " values ('$mlname', 'members-admin', '$addr', 0, '$NULL') ";
	&__Execute($mib, $query) || return $NULL;
	...
	$query  = " delete from ml ";
	$query .= " where mlname = '$mlname' ";
	$query .= " and mlfile = 'members-admin' ";
	$query .= " and address = '$addr' ";
	&__Execute($mib, $query) || return $NULL;
...
sub Count
{
    ...
    $query .= " where mlfile = '$mlfile' ";
    $query .= " and mlname = '$mlname' ";
    ($res = &__Execute($mib, $query)) || return $NULL;
...
{
    my ($mib, $mlfile) = @_;
    my ($ml, $query, $res, $addr);

    $addr   = $mib->{'_address'};
    $addr   = &main::LowerDomain($addr);
    $mlname = $mib->{'_ml_acct'};
    $query  = " select address,mloff,options from ml ";
    $query .= " where mlfile = 'actives' ";
    $query .= " and mlname = '$mlname' ";
    $query .= " and address = '$addr' ";
    ($res = &__Execute($mib, $query)) || return $NULL;

    &Log($query) if $debug;
    my ($a, $mloff, $option) = $res->fetchrow_array;

    $mib->{'_result'} .= "off "  if $mloff;
...
sub Dump
{
    ...
    print "---- table ml -----\n";
    $res = &__Execute($mib, "select * from ml order by mlfile") ||
	return $NULL;
    while (@row = $res->fetchrow_array) { 
