...
#                コピーして下さい。
#
# 使用例 : require 'mimer.pl';
#          $from = "From: Noboru Ikuta / =?ISO-2022-JP?B?GyRCQDhFRBsoQg==?=";
#          $from .= "\n\t=?ISO-2022-JP?B?GyRCPjobKEI=?= <ikuta@crc.co.jp>";
#          print &mimedecode($from, "EUC");
#
# mimedecode:
#   encoded-word（"=?ISO2022-JP?B?" と "?=" に囲まれた文字列）をサーチ
#   してデコードする。第２パラメータとして "EUC" または "SJIS" を指定
#   するとデコードした部分の漢字コードを選択的に漢字変換する。
...
## MIME デコーディング
sub mimedecode {
    local($bin) = join('', @code{split(//, shift)});
    pack("B".(length($bin)>>3<<3), $bin);
}
